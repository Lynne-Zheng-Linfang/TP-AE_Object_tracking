...
def load_info(path):
    with open(path, 'r') as f:
        ...
        for eid in list(info.keys()):
            if 'cam_K' in list(info[eid].keys()):
                info[eid]['cam_K'] = np.array(info[eid]['cam_K']).reshape((3, 3))
            if 'cam_R_w2c' in list(info[eid].keys()):
                info[eid]['cam_R_w2c'] = np.array(info[eid]['cam_R_w2c']).reshape((3, 3))
            if 'cam_t_w2c' in list(info[eid].keys()):
...
def save_info(path, info):
    for im_id in sorted(info.keys()):
        im_info = info[im_id]
        if 'cam_K' in list(im_info.keys()):
            im_info['cam_K'] = im_info['cam_K'].flatten().tolist()
        if 'cam_R_w2c' in list(im_info.keys()):
            im_info['cam_R_w2c'] = im_info['cam_R_w2c'].flatten().tolist()
        if 'cam_t_w2c' in list(im_info.keys()):
...
def load_gt(path):
    with open(path, 'r') as f:
        ...
        for im_id, gts_im in list(gts.items()):
            for gt in gts_im:
                if 'cam_R_m2c' in list(gt.keys()):
                    gt['cam_R_m2c'] = np.array(gt['cam_R_m2c']).reshape((3, 3))
                if 'cam_t_m2c' in list(gt.keys()):
...
def save_errors(path, errors):
    with open(path, 'w') as f:
        ...
        for e in errors:
            txt_errors_elems = []
            for gt_id, error in list(e['errors'].items()):
...
def save_ply(path, pts, pts_colors=np.array([]), pts_normals=np.array([]), faces=np.array([])):
    ...
    for pt_id, pt in enumerate(pts):
        if not np.isnan(np.sum(pt)):
            ...
            f.write('\n')
    for face in faces:
        f.write(' '.join(map(str, list(map(int, [len(face)] + list(face.squeeze()))))) + ' ')
        f.write('\n')
    f.close()
