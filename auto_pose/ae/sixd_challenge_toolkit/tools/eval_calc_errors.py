...
start_scene_id = arguments.scene
# Results for which the errors will be calculated
#-------------------------------------------------------------------------------
result_base = '/home/linfang/Documents/Code/AAE_tracker/ws1/test_results/Reinit_001_GRU_05_gt_rpass/vis_r_res/' 
result_paths = [
    # pjoin(result_base, 'hodan-iros15_hinterstoisser'),
    pjoin(result_base, 'PriAAE_tless_primesense'),
]
...
for result_path in result_paths:
    ...
    for scene_dir in scene_dirs:
        ...
        if os.path.exists(errors_path):
            resuts_error= inout.load_errors(errors_path)
            start_im_id = resuts_error[-1]['im_id']
        else:
            start_im_id = 0
        ...
        for res_id, res_path in enumerate(res_paths):
            ...
            im_id_prev = im_id
            im_id, obj_id = list(map(int, filename.split('_')))
            if im_id <= start_im_id:
                continue
            if (im_id_prev != im_id) and (im_id_prev != -1):
                errors_path = errors_mpath.format(result_path=result_path,
                                                  result_name=result_name,
                                                  error_sign=error_sign,
                                                  scene_id=scene_id)
                misc.ensure_dir(os.path.dirname(errors_path))
                inout.add_errors(errors_path, errs)
                errs = []
