...
# from pysixd import inout, misc, visibility
# from pysixd.wrapped_renderer import Renderer
...
dataset_part = 'test'
# import glob
model_path = '/home/linfang/Documents/Dataset/t-less_toolkit/t-less/t-less_v2/models_cad/*.ply'
# model_paths = glob.glob('/home/linfang/Documents/Dataset/t-less_toolkit/t-less/t-less_v2/models_cad/*.ply')
# model_paths.sort()
store_path = '/home/linfang/Documents/Code/AAE_tracker/ws1/tmp_datasets'
# renderer = Renderer(model_path, store_path)
use_image_subset = False # Whether to use only the specified subset of images
...
for data_id in data_ids:
    ...
    if os.path.exists(res_path):
        existing_states = inout.load_yaml(res_path)
        im_start_id = len(existing_states)
    else:
        im_start_id = 0
    for im_id in im_ids:
        if im_id < im_start_id:
            continue
        ...
        for gt_id, gt in enumerate(gts[im_id]):
            depth_gt = renderer.render(models[gt['obj_id']], im_size, K,
                                       ...
                                       mode='depth')
            # rgb, depth_gt = renderer.render_image(gt['obj_id']-1, depth_im.shape[0], depth_im.shape[1],gt['cam_R_m2c'], gt['cam_t_m2c'], K)
            # print(gt['obj_id'])
            # import cv2
            # cv2.imshow('rgb', rgb)
            # cv2.waitKey(10000)
        ...
        inout.add_yaml(res_path, states)
    exit()
