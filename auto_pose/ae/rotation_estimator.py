...
class RotationEstimator(object):
    def __init__(self,experiment_name, dataset_path, args,per_process_gpu_memory_fraction, ckpt_dir, is_training=False, phase_2 = False):
        ...
        with self.sess.as_default():
            with self.graph.as_default():
                self.dataset, self.rot_autoencoder, self.training, self.saver, \
                    self.x, self.target, self.train_op, self.queue = factory.build_rotation_estimator_architecture(experiment_name, \
                    dataset_path, args, is_training= self._is_training, phase_2 = self._phase_2)
