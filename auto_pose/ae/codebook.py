...
class Codebook(object):
    ...
    def __init__(self, encoder, dataset, embed_bb):
        ...
        self.rgb_embedding_normalized = tf.Variable(
            np.zeros((embedding_size, J)),
            dtype=tf.float32,
            trainable=False,
            name='rgb_embedding_normalized'
        )

        self.embedding = tf.compat.v1.placeholder(tf.float32, shape=[embedding_size, J])
        self.embedding_assign_op = tf.compat.v1.assign(self.embedding_normalized, self.embedding)
        self.rgb_embedding_assign_op = tf.compat.v1.assign(self.rgb_embedding_normalized, self.embedding)
        ...
        if embed_bb:
            ...
            self.embed_obj_bbs = tf.compat.v1.placeholder(tf.int32, shape=[embedding_size, 4])
            self.embed_obj_bbs_assign_op = tf.compat.v1.assign(self.embed_obj_bbs_var, self.embed_obj_bbs)
        ...
        self.rgb_cos_similarity = tf.matmul(self.normalized_embedding_query, self.rgb_embedding_normalized,transpose_b=True)
        self.nearest_neighbor_idx = tf.argmax(input=self.cos_similarity, axis=1)
        self.rgb_nearest_neighbor_idx = tf.argmax(input=self.rgb_cos_similarity, axis=1)

    def nearest_rotation(self, session, x, top_n=1, upright=False, return_idcs=False, return_similarity_score= False, return_confidence = False):
        #R_model2cam
        ...
        if top_n == 1:
            ...
            confidence = np.max(cosine_similarity, axis=1)
        else:
        ...
        if return_idcs:
            return idcs
        elif return_similarity_score and return_confidence:
            return self._dataset.viewsphere_for_embedding[idcs].squeeze(), cosine_similarity, confidence 
        elif return_similarity_score:
            return self._dataset.viewsphere_for_embedding[idcs].squeeze(), cosine_similarity
        else:
            return self._dataset.viewsphere_for_embedding[idcs].squeeze()

    def rgb_nearest_rotation(self, session, x, top_n=1, upright=False, return_idcs=False, return_similarity_score= False, return_confidence = False):
        #R_model2cam
        ...
        cosine_similarity = session.run(self.rgb_cos_similarity, {self._encoder.x: x})
        if top_n == 1:
            if upright:
                idcs = np.argmax(cosine_similarity[:,::int(self._dataset._kw['num_cyclo'])], axis=1)*int(self._dataset._kw['num_cyclo'])
            else:
    ...
    def get_rotation_from_code(self, session, code):
        cosine_similarity = session.run(self.cos_similarity, {self._encoder.z: code})
        idcs = np.argmax(cosine_similarity, axis=1)
        return self._dataset.viewsphere_for_embedding[idcs].squeeze()
    ...
    def update_embedding(self, session, batch_size):
        ...
        J = self._encoder.latent_space_size
        embedding_z = np.empty( (embedding_size, J) )
        rgb_embedding_z = np.empty( (embedding_size, J) )
        obj_bbs = np.empty( (embedding_size, 4) )

        bar = u.progressbar_init('Creating embedding ..: ', embedding_size)
        ...
        for a, e in u.batch_iteration_indices(embedding_size, batch_size):
            ...
            batch[:,:,:, 3:6] = 0
            rgb_embedding_z[a:e] = session.run(self._encoder.z, feed_dict={self._encoder.x: batch})
            
            # cv2.imshow('batch_0_rgb', batch[0,:,:,0:3] )
            # cv2.imshow('batch_1_depth', batch[0,:,:,3:] )
            # cv2.waitKey(20000)
            # exit()
            if self.embed_bb:
        ...
        # embedding_z = embedding_z.T
        normalized_embedding = embedding_z / np.linalg.norm( embedding_z, axis=1, keepdims=True )
        rgb_normalized_embedding = rgb_embedding_z / np.linalg.norm( rgb_embedding_z, axis=1, keepdims=True )

        session.run(self.embedding_assign_op, {self.embedding: normalized_embedding})
        session.run(self.rgb_embedding_assign_op, {self.embedding: rgb_normalized_embedding})
