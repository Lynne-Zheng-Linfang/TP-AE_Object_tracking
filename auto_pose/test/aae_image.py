...
codebook, predictor, R_encoder, R_decoder, t_encoder, t_decoder, dataset, input_x = factory.build_inference_architecture(experiment_name, dataset_path, args)
...
if ordered_eval:
    ...
    if syn_eval:
        ...
        test_data, test_delta_t, test_delta_R, test_gt_t, test_gt_R, test_noisy_R= dataset.load_eval_images(dataset_path, args)
...
with tf.compat.v1.Session(config=config) as sess:
    ...
    if t_output or ordered_eval:
        ...
        for i in range(t_iter_num):
            ...
            for iteration in range(recursive_num):
                if not class_t:
                    ...
                    coarse_delta_t = sess.run(predictor.coarse_delta_t_out,feed_dict={input_x: new_x})
                    ...
                    fine_delta_t = sess.run(predictor.fine_delta_t_with_code_out,feed_dict={input_x: new_x})
                    ...
                    delta_t = np.array(sess.run(predictor.t_out, feed_dict={input_x: new_x}))
        ...
        yaxes = 'image num'
        titles = ['fine translation prediction error (code)', 'fine translation prediction error (t)','coarse translation error'] 
        ...
        for i in range(test_num):
            ...
            reconstr_train = sess.run(R_decoder.x,feed_dict={R_encoder.x: this_x})
            new_x = this_x[:,:,:,0:6].copy()
